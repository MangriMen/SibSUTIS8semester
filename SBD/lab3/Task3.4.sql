...
CREATE OR REPLACE PACKAGE BODY caller IS
  ...
  PROCEDURE print IS
  BEGIN
    shorterSurname := lib.shorter_surname('Лёвкин', 'Панин');
    longerSurname := lib.longer_surname('Лёвкин', 'Панин');

    dbms_output.put_line('Самая короткая из фамилий: ' || shorterSurname);
    dbms_output.put_line('Самая длинная из фамилий: ' || longerSurname);
  END print;
END caller;
/
